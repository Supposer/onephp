  ...
  public function EncodeQ ($str, $position = 'text') {
    ...
    switch (strtolower($position)) {
      case 'phrase':
        ...
        $encoded = preg_replace("/([^A-Za-z0-9!*+\/ -])/", function($matches) {return '='.sprintf('%02X', ord($matches[1]));}, $encoded);
        break;
      case 'comment':
        //$encoded = preg_replace("/([\(\)\"])/e", "'='.sprintf('%02X', ord('\\1'))", $encoded);
        $encoded = preg_replace("/([\(\)\"])/", function($matches) {return '='.sprintf('%02X', ord($matches[1]));}, $encoded);
      case 'text':
      default:
